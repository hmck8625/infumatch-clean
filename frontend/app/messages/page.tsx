...
function MessagesPageContent() {
  ...
  const processNewThread = async (threadId: string) => {
    try {
      ...
      // 前回送信したメッセージIDをチェック
      const lastSentMessageId = trackedThreads[threadId]?.lastSentMessageId;
      
      if (lastSentMessageId && latestMessageId === lastSentMessageId) {
        console.log('⚠️ 最新メッセージは自分が送信したメッセージのためスキップ:', {
          メッセージID: latestMessageId,
          送信済みID: lastSentMessageId
        });
        return;
      }
